...
echo "dpkg --configure -a && apt-get install -f && apt-get --purge autoremove -y && apt-get update && apt-get upgrade -y && apt-get dist-upgrade -y && apt-get autoremove -y && apt-get autoclean -y" >> ~/update.sh
